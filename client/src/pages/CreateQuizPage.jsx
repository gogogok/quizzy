...
export default function CreateQuizPage() {
  ...
  function loadImage(file) {
    ...
    reader.onload = () => {
      updateQuestion('imageUrl', reader.result);
      setError('');
    };
  ...
  function removeQuestion(index) {
    ...
    setSelected((value) =>
      Math.max(0, value > index ? value - 1 : Math.min(value, questions.length - 2)),
    );
  }
        ...
        <section className="editor">
          {currentStep === 0 && (
            <>
              ...
              <div className="field">
                ...
                <textarea
                  className="input builder-textarea"
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Коротко расскажите о квизе"
                />
              </div>
              <div className="field">
                ...
                <input
                  ...
                  placeholder="Общие знания"
                />
              </div>
              <button className="btn primary next-step" onClick={() => setCurrentStep(1)}>
                Далее: вопросы →
              </button>
            </>
          )}
          ...
          {currentStep === 1 && (
            <>
              <div className="builder-section-head">
                <div>
                  <h3>Вопросы</h3>
                  <p>{questions.length} вопросов в квизе</p>
                </div>
                <button
              ...
              {questions.map((item, index) => (
                <div
                  className={`question ${index === selected ? 'selected-question' : ''}`}
                  onClick={() => setSelected(index)}
                  key={index}
                >
                  <div>
                    <b>
                      {index + 1}. {item.text || 'Новый вопрос'}
                    </b>
                    <p>
                      {item.type === 'SINGLE' ? 'Одиночный' : 'Множественный'} выбор ·{' '}
                      {item.timeLimit} сек.
                    </p>
                  </div>
                  <button
              ...
              <button className="btn next-step" onClick={() => setCurrentStep(2)}>
                Далее: настройки →
              </button>
            </>
          )}
          ...
          {currentStep === 2 && (
            <>
              ...
              <div className="field">
                <label>Время на вопрос по умолчанию</label>
                <input
                  className="input"
                  type="number"
                  min="5"
              ...
              <div className="setting-note">
                <b>Баллы за скорость</b>
                <p>
                  Правильный ответ даёт минимум 50% баллов. Остальная часть зависит от скорости.
                </p>
              </div>
              <div className="setting-note">
                <b>Порядок вопросов</b>
                <p>Вопросы отображаются в том порядке, в котором находятся в списке.</p>
              </div>
              <button className="btn primary next-step" onClick={() => setCurrentStep(3)}>
                Далее: публикация →
              </button>
            </>
          )}
          ...
          {currentStep === 3 && (
            <>
              ...
              <div className="publish-summary">
                <span>Название</span>
                <b>{title || 'Не указано'}</b>
                <span>Категория</span>
                <b>{category || 'Не указана'}</b>
                <span>Вопросов</span>
                <b>{questions.length}</b>
              </div>
              <div className="field">
                <label>Статус квиза</label>
                <select
                  className="input"
                  value={status}
              ...
              <button className="btn primary wide" disabled={busy} onClick={save}>
                {busy ? 'Сохраняем…' : id ? 'Сохранить изменения' : 'Создать квиз'}
              </button>
            </>
          )}
        </section>
        ...
        {currentStep === 1 && question && (
          <aside className="settings">
            ...
            <div className="question-kind-switch">
              <button
                type="button"
                className={question.imageUrl === null ? 'active' : ''}
              ...
              <button
                type="button"
                className={question.imageUrl !== null ? 'active' : ''}
                onClick={() => updateQuestion('imageUrl', question.imageUrl || '')}
              >
                ▧ С изображением
              </button>
            </div>
              ...
              <div className="image-editor">
                ...
                <div className="field">
                  ...
                  <input
                    className="input"
                    type="url"
                    value={question.imageUrl?.startsWith('data:') ? '' : question.imageUrl || ''}
            ...
            <div className="options-heading">
              <label className="options-label">Варианты ответа ({question.options.length})</label>
              <span>от 2 до 8</span>
            </div>
            {question.options.map((option, index) => (
