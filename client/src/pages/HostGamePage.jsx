...
export default function HostGamePage() {
  ...
  useEffect(() => {
    emitAck('host:join', { roomCode })
      .then((result) => {
        setLeaders(result.participants);
        if (result.game) {
          setGame(result.game);
          setClosed(false);
        }
  ...
  async function close() {
    try {
      await emitAck('question:close', { roomCode });
    } catch (requestError) {
    ...
    <div className="dark-game">
      <div className="game-top">
        ...
        <div className="actions">
          <button onClick={close} disabled={!game || closed} className="btn">
            Закрыть вопрос
          </button>
      ...
      <div className="game-grid">
        <section className="question-panel">
          {game ? (
            <>
              ...
              <div className="game-card">
                ...
                {closed && (
                  <div className="question-closed-note">
                    Вопрос закрыт — ответы больше не принимаются
                  </div>
                )}
              </div>
            </>
          ...
          <button
            ...
            style={{ marginTop: 15 }}
          >
            {game ? 'Следующий вопрос' : 'Показать первый вопрос'}
          </button>
        </section>
